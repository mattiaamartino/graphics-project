...
public class SurvivorSpawner : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Spawn(humanPrefab, humanCount);
        Spawn(predatorPrefab, predatorCount);
        Spawn(preyPrefab, preyCount);
        Spawn(vegetablePrefab, vegetableCount);
    }
    ...
    void Spawn(GameObject prefab, int count)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            float y = terrain != null ? terrain.getInterp(x, z) : 0;
